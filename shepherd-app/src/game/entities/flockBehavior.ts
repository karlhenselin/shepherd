...
export type FlockTraits = {
    ...
    waddlePhase: number;
    /** After the change, Biscuit matches adult sheep speed. */
    grownFollowSpeed?: number;
};
...
export type FlockMood = 'waiting' | 'following' | 'drinking' | 'eating' | 'hurt' | 'penned';
export type FlockEvent = 'found' | 'ate' | 'drank' | 'rejoined' | null;
...
export class FlockBehavior {
    ...
    private readonly appearance: FlockAppearance;
    ...
    constructor (
        ...
        this.peaceable = peaceable;

        this.shadow = scene.add.image(x, y + appearance.shadowOffset, appearance.shadowKey);

        if (appearance.shadowDisplaySize) {
            this.shadow.setDisplaySize(appearance.shadowDisplaySize.width, appearance.shadowDisplaySize.height);
        }

        this.sprite = scene.physics.add.sprite(x, y, appearance.textureKey);

        if (appearance.fitAspect) {
            const src = this.sprite.texture.getSourceImage() as { width: number; height: number };
            const height = appearance.displayHeight;
            const width = height * (src.width / Math.max(src.height, 1));
            this.sprite.setDisplaySize(width, height);
        }
        else {
            this.sprite.setDisplaySize(appearance.displayHeight, appearance.displayHeight);
        }

        this.sprite.setOrigin(0.5, appearance.originY);

        if (appearance.tint !== undefined) {
            this.sprite.setTint(appearance.tint);
        }
    ...
    private applyFlockTint (): void {
        if (this.appearance.tint !== undefined) {
            this.sprite.setTint(this.appearance.tint);
            return;
        }
    ...
    get strayWeight (): number {
        return this.appearance.traits.strayWeight;
    }
    ...
    update (
        ...
        if (this.mood === 'penned') {
            if (this.penTarget) {
                const gap = Math.hypot(this.penTarget.x - this.sprite.x, this.penTarget.y - this.sprite.y);
    ...
    private trailTarget (shepherd: Shepherd, huddle: boolean): { targetX: number; targetY: number } {
        const spacing = (huddle && !this.changed ? NIGHT_FOLLOW_DISTANCE : this.changed ? FOLLOW_DISTANCE + 15 : FOLLOW_DISTANCE)
            * this.appearance.traits.trailScale;
        const { x: hx, y: hy } = shepherd.moveHeading;
        const behindX = -hx;
        const behindY = -hy;
        const perpX = -hy;
    ...
    private followSpeedScale (): number {
        if (this.changed && this.appearance.traits.grownFollowSpeed !== undefined) {
            return this.appearance.traits.grownFollowSpeed;
        }

        return this.appearance.traits.followSpeed;
    }
    ...
    private waddleAngle (now: number): number {
        const traits = this.appearance.traits;
    ...
    private placeShadow (): void {
        ...
        const tilt = this.appearance.shadowTiltDeg;

        if (tilt) {
            this.shadow.setAngle(this.sprite.flipX ? -tilt : tilt);
        }
    }
